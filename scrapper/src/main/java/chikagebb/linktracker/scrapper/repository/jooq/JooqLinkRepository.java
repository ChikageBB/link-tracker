...
public class JooqLinkRepository implements LinkRepository {
    ...
    public LinkDto save(Long chatId, URI url, List<String> tags) {
        ...
        for (String tag : tags) {
            ...
            dslContext
                    .insertInto(table("chat_link_tags"))
                    .columns(field("chat_id", Long.class), field("link_id", Long.class), field("tag_id", Long.class))
                    .select(select(val(chatId), val(linkId), field("id", Long.class))
                            .from(table("tags"))
    ...
    public List<LinkDto> findAll(Long chatId, int page, int size) {
        ...
        return dslContext
                ...
                .on(field("cl.link_id").eq(field("l.id")))
                ...
                .leftJoin(table("tags").as("t"))
                .on(field("t.id").eq(field("clt.tag_id")))
    ...
    public boolean existByUrl(Long chatId, URI url) {
        return dslContext.fetchExists(selectOne()
                .from(table("chat_links").as("cl"))
                .join(table("links").as("l"))
                .on(field("cl.link_id").eq(field("l.id")))
    ...
    public boolean delete(Long chatId, URI url) {
        int rowsDeleted = dslContext
                .deleteFrom(table("chat_links"))
    ...
    public Map<Long, List<LinkDto>> findAllLinkSubscriber(int page, int size) {
        ...
        return dslContext
                ...
                .leftJoin(table("tags").as("t"))
                .on(field("t.id").eq(field("clt.tag_id")))
                .groupBy(field("l.id"), field("l.url"), field("l.last_check_at"), field("cl.chat_id"))
                .limit(size)
                .offset(page * size)
                .fetchGroups(r -> r.get("chat_id", Long.class), this::toDto);
    }
    ...
    public List<LinkDto> findOldestChecked(int page, int size) {
        ...
        return dslContext
                ...
                .on(field("t.id").eq(field("clt.tag_id")))
                .groupBy(
                        field("l.id", Long.class),
                        field("l.url", String.class),
